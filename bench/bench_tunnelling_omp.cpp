...
static void TunnellingOpenMP_Rosenbrock(benchmark::State& state) {
	...
	const ObjectiveFunction& r = Rosenbrock();

	for (auto _ : state) {
		const auto start = std::chrono::high_resolution_clock::now();

		const std::pair<std::vector<double>, double> result = 
		algorithm::run_multi_stochastic_tunnelling(dimensions, max_iterations, seed, lower_bound, upper_bound, sigma_max, sigma_min, r, gamma, beta_adjust_factor, false, beta, tunnelling, beta_tresholding, num_positions, time_step_updating);
