...
DifferentialEvolution::DifferentialEvolution(const std::vector<Candidate>& candidates_, const size_t dimensions_, const double lower_bound_, const double upper_bound_, const size_t seed_, const size_t max_gen_, const double F_, const double CR_, ObjectiveFunction& func_, const size_t n_threads_) noexcept
    : candidates(candidates_), dimensions(dimensions_),lower_bound(lower_bound_),upper_bound(upper_bound_), seed(seed_),F(F_), CR(CR_),max_gen(max_gen_) , func(func_), n_threads(n_threads_),gens(n_threads_){
...
void DifferentialEvolution::select_three_random(const int excluded_index,int& i1, int& i2, int& i3, std::mt19937& local_gen) const {
...
void DifferentialEvolution::mutate(std::vector<double>& mutantPos, const Candidate &r1, const Candidate &r2, const Candidate &r3) const {
...
void DifferentialEvolution::crossover(Candidate& original,const Candidate& mutant, std::mt19937& local_gen) const {
    std::vector<double> mergedPos= original.candidate;
    std::uniform_int_distribution<int> intDist(0,static_cast<int>(dimensions - 1));
...
#pragma omp for schedule(static)
        for (int i= 0; i < static_cast<int>(candidates.size()); i++) {
            ...
            const Candidate& r1 = candidates[i1];
            const Candidate& r2 = candidates[i2];
            const Candidate& r3 = candidates[i3];
...
void DifferentialEvolution::findSol() noexcept {
    double minValue = std::numeric_limits<double>::infinity();
    for (std::size_t i = 0; i < candidates.size(); ++i) {
        if (candidates[i].f0 < minValue) {
            bestIndex = i;
            minValue = candidates[i].f0;
        }
    }
}

const Candidate& DifferentialEvolution::getBestCandidate() const noexcept {
